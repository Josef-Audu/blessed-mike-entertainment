...
export default function ContactPressPage() {
  return (
    <div className="bg-zinc-950 text-white">
      <section className="border-b border-zinc-900 px-5 py-24 sm:px-8 md:py-28">
        <Reveal className="mx-auto max-w-7xl">
          <p className="text-xs font-black uppercase tracking-[0.28em] text-amber-500">Company / Press room</p>
          <div className="mt-5 grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            <h1 className="font-sports text-5xl font-bold uppercase leading-none sm:text-7xl">Let&apos;s make<br /><span className="text-zinc-500">the headline.</span></h1>
            <p className="max-w-xl leading-8 text-zinc-400">For interviews, editorial partnerships, event accreditation, corrections, and licensing requests, send our press desk the details below.</p>
          </div>
        </Reveal>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[0.72fr_1.28fr] lg:py-28">
        ...
        <Reveal delay={0.08}>
          <form action="mailto:press@blessedmikesentertainment.com" method="post" encType="text/plain" className="rounded-3xl border border-zinc-800 bg-zinc-900/20 p-6 shadow-2xl shadow-black/30 sm:p-9">
            ...
            <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button type="submit" className="rounded-full bg-amber-500 px-6 py-3 text-xs font-black uppercase tracking-[0.18em] text-zinc-950 transition hover:-translate-y-1 hover:bg-amber-300 hover:shadow-[0_14px_40px_rgba(245,158,11,0.2)]">Prepare inquiry</button>
            </div>
          </form>
        </Reveal>
      </section>
